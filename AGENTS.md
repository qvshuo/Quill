...
- Sync feedback = transient top-center **toast** (`SyncToastView`, piped by `InputState.toast` of type `SyncToast` in `Models`): `started` persists until the sync finishes (`SyncToast` carries no duration), then the controller swaps in `同步完成` / `同步超时，仍在后台进行` / `同步失败，请检查设置` and dismisses after 2.5 s (completed) / 4.0 s (timedOut, failed) (`InputController.scheduleToastDismissal`). The hold-hint capsule and the toast share one position and one visual language (capsule, `keyBackground` fill + `floatingShadow()`, mutually exclusive via `SyncToastOverlay.phase`), `allowsHitTesting(false)`, floats at keyboard top center.
...
- Sync is bounded by a **30 s timeout** (`WebDAVSync.syncWithTimeout(.seconds(30))`). The timeout **races** the sync and never cancels it: on timeout the toast shows `.timedOut`（「同步超时，仍在后台进行」）while the zombie sync keeps running and typically completes its uploads — a 2026-08-23 incident (slow new WebDAV server) showed "files uploaded but 同步失败" because the old code mapped timeout to plain failure. Only a genuine error maps to `.failed`. The `recreateSession()` call itself is owned by `InputController.startManualSync()` and runs whenever `preedit.isEmpty` **regardless of outcome** (queued behind any zombie via the sync serial queue) — a failed/timeout sync may still have applied foreign `custom_phrase.txt` (step 3.5), which needs a session reload to take effect.
- **No-App-Group is the supported baseline, not a defect**: the app is designed around self-signed installs (user uses Feather); the main app and keyboard are expected to run with independent sandboxes, user data, and logs. Each process logs an `AppGroup shared/per-app` marker at startup so you know where its log lands; keyboard-side sync logs land in the extension's private `Documents/Logs` and never appear in the main app's 导出日志 — that is normal, do not "fix" it or warn about it in the UI. Status flags that ride the shared defaults (e.g. `hasFullAccess`) are unreadable in per-app mode — the settings page only shows such warnings when `Paths.appGroupContainer != nil`.
...
- The WebDAV settings are **one 同步 section**: 服务器地址/用户名/密码/同步目录/安装 ID each get a footnote label above via `LabelledField`, then 保存/测试连接; the footer shows 最近同步于 + the 长按空格 3 秒 hint. Input fields use the `clearableField` modifier (gray X, `.tint(.secondary)`, shown only when focused & non-empty); the info `info.circle` is `.tint(.primary)` (a `.foregroundStyle` on the image is overridden by the button tint).
...
1. **In-flight guard**: a process-wide `Mutex` marks a sync in progress. A concurrent trigger (including a zombie sync still finishing after the 30 s timeout lost the race) is skipped and returns `false`. Never run two syncs in parallel — they would wipe/read/write the same staging dir and user dict.
