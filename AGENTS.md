...
- Press feedback is a **uniform pressed fill** via `Theme.fillColor(style:isPressed:)` — every key style (`normal`/`special`/`confirm`) presses to the single `Theme.pressedKeyBackground`: light `#D1D4D9` (dims), dark white @0.30 ≈ `#6B6B6B` (brightens). The confirm key's foreground swaps to `keyForeground` on press so the label stays visible on the light pressed tone.
...
- **Theme follows SwiftUI `@Environment(\.colorScheme)`** (fcitx5-ios's simplest approach). `KeyboardView` picks `Theme.dark`/`Theme.light` itself. `InputController` does **not** resolve dark/light, sets no `overrideUserInterfaceStyle`, and rebuilds the root view only for `keyboardType`/`returnKeyType` changes. Do not reintroduce a `resolvedIsDark()` host heuristic (`keyboardAppearance` → trait → `UIScreen.main`) — it was removed on purpose.
...
- The space key is **not** repeatable. Tap inserts a space; double-tap within 0.35 s converts to `。` (zh) / `.` (en). **Double-tap conversion only applies when there is no RIME preedit** — during a composition the space is handed to RIME (`InputController` guards the conversion on `preedit.isEmpty`, so a quick second tap can't insert a literal `。` past RIME). **Long-press 5 s triggers the manual WebDAV sync** (`Key.spaceKeyBody` runs a 5 s `DispatchWorkItem`; `startSpaceHoldTimer` → `action(.startSync)`); releasing before 5 s is a normal space. A RIME preedit aborts it (`startManualSync()` guards `preedit.isEmpty`).
